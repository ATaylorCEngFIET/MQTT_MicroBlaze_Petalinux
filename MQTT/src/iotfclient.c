...
#define MQTT_WRITE_BUF_SIZE (1000)
#define MQTT_READ_BUF_SIZE  (1000)
static unsigned char buf[MQTT_WRITE_BUF_SIZE];
...
int connectiotf(Iotfclient *client)
{
	...
	MQTTClient(&client->c, &client->n, 1000, buf, MQTT_WRITE_BUF_SIZE, readbuf, MQTT_READ_BUF_SIZE);
...
int publishEvent(Iotfclient *client, char *eventType, char *eventFormat, unsigned char* data, enum QoS qos)
{
	...
	pub.retained = '0';
	pub.payload = data;
	pub.payloadlen = strlen((const char *)data);
...
int retry_connection(Iotfclient *client) 
{
	...
	for(i=0; (i<5) && (connectiotf(client) != SUCCESS); i++)
	{
